#!/bin/bash
#
# Script de ejecución MySQL via CRON
# AseguraloCR - Crear tablas de base de datos
#
# IMPORTANTE: Este script se ejecuta UNA VEZ y luego se auto-elimina
#

SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
LOG_FILE="$SCRIPT_DIR/resultado-mysql.txt"
LOCK_FILE="$SCRIPT_DIR/.mysql-ejecutado"

# Verificar si ya se ejecutó
if [ -f "$LOCK_FILE" ]; then
    echo "Ya ejecutado el $(cat $LOCK_FILE)" | tee -a "$LOG_FILE"
    exit 0
fi
...
echo "========================================" | tee "$LOG_FILE"
echo "AseguraloCR - Creación de Tablas MySQL" | tee -a "$LOG_FILE"
echo "========================================" | tee -a "$LOG_FILE"
echo "Inicio: $(date '+%Y-%m-%d %H:%M:%S')" | tee -a "$LOG_FILE"
echo "" | tee -a "$LOG_FILE"

# Ejecutar SQL
mysql -h "$DB_HOST" -u "$DB_USER" -p"$DB_PASS" "$DB_NAME" << 'EOSQL' 2>&1 | tee -a "$LOG_FILE"
...
-- CREAR TABLA CLIENT_NOTIFICATIONS
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "✅ EJECUCIÓN COMPLETADA EXITOSAMENTE" | tee -a "$LOG_FILE"

    # Marcar como ejecutado
    date '+%Y-%m-%d %H:%M:%S' > "$LOCK_FILE"

    # Auto-eliminar script
    echo "🗑️  Auto-eliminando script..." | tee -a "$LOG_FILE"
    rm -f "$0"
else
