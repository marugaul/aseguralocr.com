...
// autos_procesar.php - Procesador de solicitudes de seguro de automóvil
...
function log_err(string $msg): void {
    error_log('[autos_procesar] ' . $msg);
}

function generate_autos_pdf(array $clean, string $referencia, string $pdfPath): array {
    ...
    try {
        ...
        $fmtMoney = function($amount, $moneda = 'dolares') {
        ...
        $pdf->Cell(0, 8, utf8_decode('SOLICITUD DE SEGURO DE AUTOMÓVIL'), 0, 1, 'C');
        $pdf->Ln(6);
        ...
        $print_section = function(FPDF $pdf, string $title, array $fields, $getValFn, $getConsentFn, $fmtMoneyFn) use ($clean) {
            ...
            foreach ($fields as $label => $meta) {
                ...
                if ($type === 'consent') {
                    ...
                    $val = $fmtMoneyFn($clean[$key] ?? '', $clean[$meta['monedaKey'] ?? 'moneda'] ?? 'dolares');
                } else {
                    $val = $getValFn($key);
                }
        ...
        $sections = [
            ...
            'USO DEL VEHÍCULO' => [
                'Uso Principal' => 'usoVehiculo',
                'Kilometraje' => 'kilometraje',
                'Lugar de Guarda' => 'lugarGuarda',
                'Financiamiento' => 'tieneFinanciamiento',
                'Entidad Financiera' => 'entidadFinanciera',
                'Siniestros Previos' => 'siniestrosPrevios',
                'Detalle Siniestros' => 'detalleSiniestros',
                'Conductores Habituales' => 'conductoresHabituales'
            ],
            'COBERTURAS' => [
                'Valor del Vehículo' => ['key' => 'valorVehiculo', 'type' => 'money', 'monedaKey' => 'moneda'],
                'Moneda' => 'moneda',
                'Responsabilidad Civil' => 'coberturaRC',
                'Daños Propios' => 'coberturaDanosPropios',
                'Robo Total' => 'coberturaRoboTotal',
                'Robo Parcial' => 'coberturaRoboParcial',
                'Asistencia Vial' => 'coberturaAsistencia',
                'Gastos Médicos' => 'coberturaGastosMedicos',
                'Vehículo Sustituto' => 'coberturaVehiculoSustituto',
                'Eventos Catastróficos' => 'coberturaCatastrofe',
                'Rotura de Vidrios' => 'coberturaVidrios',
                'Deducible' => 'deducible',
...
try {
    ...
    // Validate required fields - Datos del Propietario/Solicitante
    ...
    $placa = sanitize((string)($in['placa'] ?? ''));
    ...
    if ($placa === '') {
        http_response_code(400);
        echo json_encode(['success' => false, 'message' => 'Placa del vehículo es obligatoria']);
        exit;
    }
    ...
    // Agregar prefijo autos_ a todas las claves para el payload del PDF mapper
    $prefixedPayload = [];
    foreach ($clean as $k => $v) {
        // No prefijar campos de sistema como csrf, website
        if (in_array($k, ['csrf', 'website'], true)) continue;
        $prefixedPayload['autos_' . $k] = $v;
    }
    ...
    $gen = generate_autos_pdf($clean, $referencia, $pdfPath);
    if (empty($gen['ok'])) {
    ...
    $linkResult = $quoteService->linkQuoteToClient($correo, $referencia, 'autos', $prefixedPayload, $pdfPath);
    ...
    if (class_exists('PHPMailer\\PHPMailer\\PHPMailer') || class_exists('PHPMailer')) {
        try {
            ...
            $m->isHTML(true);
            $m->Subject = "Nueva solicitud Seguro Auto ($referencia)";

            $vehiculoInfo = ($clean['marca'] ?? '') . ' ' . ($clean['modelo'] ?? '') . ' ' . ($clean['ano'] ?? '');
            $htmlBody = "<p>Se ha recibido una nueva solicitud de seguro de automóvil.</p>";
            $htmlBody .= "<p><b>Referencia:</b> " . htmlspecialchars($referencia) . "</p>";
            $htmlBody .= "<p><b>Solicitante:</b> " . htmlspecialchars($nombre) . "</p>";
            $htmlBody .= "<p><b>Correo:</b> " . htmlspecialchars($correo) . "</p>";
            $htmlBody .= "<p><b>Vehículo:</b> " . htmlspecialchars($vehiculoInfo) . "</p>";
            $htmlBody .= "<p><b>Placa:</b> " . htmlspecialchars($placa) . "</p>";
            $htmlBody .= "<hr><p>La información completa se encuentra en el PDF adjunto.</p>";

            $plain = "Nueva solicitud de seguro de automóvil.\n";
            $plain .= "Referencia: $referencia\n";
            $plain .= "Solicitante: $nombre\n";
            $plain .= "Vehículo: $vehiculoInfo\n";
            $plain .= "Placa: $placa\n";
